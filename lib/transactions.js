...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    //zcoin
     if (rpcData.znode) {
        ...
        var znode = rpcData.znode;

        var testnet = poolOptions.testnet;
        ...
        Object.keys(founders).forEach((founderAddress) => {
            var founderReward = founders[founderAddress] * denomination;

            var rewardScript = util.addressToScript(founderAddress);
            txOutputBuffers.push(Buffer.concat([
